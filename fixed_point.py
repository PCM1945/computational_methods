...
class Point:
    ...
    def beta(self, x):
        ...
        return (eval(self.beta_func)) # translates a  valid expession into python code
    ...
    def fixed_point(self):
        ...
        f = File(["iter",'x0', 'f(x)', "b(x)", "|f(x)|", '|xn-xn-1|'], 'fixed.csv')
        f.create_csv()
        data = []
        while True:
            try:
                ...
                print("fixed point method diverged with given interval (float overflow) \nlast iteration:")
                print(f"Root = {x1}")
                print(f"Interactions = {interaction}")
                break
