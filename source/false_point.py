...
class Sec:
    ...
    def point(self):
        i = 0
        f = File(['iter', 'xa', 'f(xa)', 'xb', 'f(xb)', '|xn-xn-1|',
                 '|f(x)|', '|f(xa)|', '|f(xa)|'], 'false.csv')
        f.create_csv()
        data = []
        a = self.interval[0]
        b = self.interval[1]
        while True:
            try:
                ...
                # calculates approximated zero
                ez = ((a * fxb) - (b * fxa)) / (fxb - fxa)
                fx = self.f(ez)

                data.extend([i, a, fxa, b, fxb, abs(b - a),
                            abs(fxa), abs(fxb), abs(fx)])
                f.write_file(data)
                data.clear()

                if abs(
                        b - a) < self.precision:  # the root is the medium point of [a,b]
                    ez = ((b + a) / 2)
                ...
                print(
                    "false point method diverged with given interval (float overflow) \nlast iteration:")
                print(f"Root = {ez}")
