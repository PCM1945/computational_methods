...
class Bissection:
    ...
    def bisect(self):
        f = File(['interections', 'Xa', 'f(xa)', 'Xb', 'f(xb)', 'xn',
                 'f(xn)', '|fxn|', '|xn- xn-1|'], 'bisection.csv')
        f.create_csv()
        file_data = []
        ...
        if abs(
                ia -
                ib) < self.precision:  # interval ib is smaller than precision -> root = (a+b)/2
            print("root:", (ia + ib) / 2)
            ...
            while True:
                try:
                    ...
                    f_x = self.fx(x)  # f(x)
                    file_data.extend(
                        [interections, ia, fx_a, ib, fx_b, x, f_x, abs(f_x), abs(ib - ia)])
                    ...
                    if abs(f_x) < self.precision:  # se f(x) < precision root = x
                        ...
                        break
                    if abs(
                            ib - ia) < self.precision:  # interval[a,b] < precision -> root = (a+b)/2
                        x = (ia + ib) / 2
                        print(f"Root: {x}")
                        print(f"Interections = {interections + 1}")
                    ...
                    print(
                        "bisection method diverged with given interval (float overflow) \nlast iteration:")
                    print(f"Root = {x}")
